...
Generate only the cover letter content, no additional commentary.
  ...
  displayFormScanResults(formInfo) {
    ...
    if (resultElement) {
      const detectedFieldsText =
        formInfo.detectedFields.length > 0
          ? formInfo.detectedFields
              .slice(0, 3)
              .map((field) => field.type)
              .join(", ") + (formInfo.detectedFields.length > 3 ? "..." : "")
          : "None detected";
  ...
  injectCoverLetter(coverLetterText) {
    ...
    if (coverLetterInputs.length === 0) {
      ...
      const largeTextareas = Array.from(document.querySelectorAll("textarea")).filter(
        (ta) => ta.getAttribute("maxlength") > 500 || !ta.getAttribute("maxlength")
      );
